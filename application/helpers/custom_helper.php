...
if (!function_exists('get_logo')) {
    function get_logo($settings)
    {
        ...
        return base_url() . "assets/img/logo1.svg";
    }
}
...
//get cart product
if (!function_exists('get_cart_product')) {
    function get_cart_product($id)
    {
        $ci =& get_instance();
        return $ci->product_model->get_cart_product($id);
    }
}
...
if (!function_exists('get_slider_image_url')) {
    function get_slider_image_url($slide)
    {
        if ($slide->storage == "aws_s3") {
            $ci =& get_instance();
            return $ci->aws_base_url . $slide->image;
        } else {
            return base_url() . $slide->image;
        }
    }
}
...
if (!function_exists('get_cart_product_count')) {
    ...
    {
        if (isset($_SESSION["mds_shopping_cart"])) {
            return count($_SESSION["mds_shopping_cart"]);
        } else {
            return 0;
        }
    }
...
if (!function_exists('get_sess_custom_field_options_array')) {
    function get_sess_custom_field_options_array()
    {
        $ci =& get_instance();
        return $ci->field_model->get_sess_custom_field_options_array();
    }
}

//get sess custom fields category array
if (!function_exists('get_sess_custom_fields_category_array')) {
    function get_sess_custom_fields_category_array()
    {
        $ci =& get_instance();
        return $ci->field_model->get_sess_custom_fields_category_array();
    }
}
...
if (!function_exists('get_twitter_login_url')) {
    function get_twitter_login_url()
    {
        if (!auth_check()) {
            $ci =& get_instance();
            try {
                if (!empty($ci->general_settings->twitter_api_key) && !empty($ci->general_settings->twitter_secret_key)) {
                    $ci->load->library("twitter");
                    return $ci->twitter->generate_login_url();
                }
            } catch (Exception $e) {
            }
        }
...
//get sess modesy sort products
if (!function_exists('get_sess_modesy_sort_products')) {
    function get_sess_modesy_sort_products()
    {
        $ci =& get_instance();
        if (!empty($ci->session->userdata('modesy_sort_products'))) {
            return $ci->session->userdata('modesy_sort_products');
        }
    }
}
