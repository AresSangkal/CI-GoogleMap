...
class Order_admin_controller extends Admin_Core_Controller
{
    ...
    public function order_options_post()
    {
        ...
        if ($option == "completed") {
            $this->order_admin_model->set_order_as_completed($order_id);
        } elseif ($option == "not_completed") {
            $this->order_admin_model->set_order_as_not_completed($order_id);
        } elseif ($option == "payment_received") {
            $this->order_admin_model->update_order_payment_received($order_id);
        } elseif ($option == "payment_not_received") {
            $this->order_admin_model->update_order_payment_not_received($order_id);
        }
