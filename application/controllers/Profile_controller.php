...
class Profile_controller extends Home_Core_Controller
{
    ...
    public function profile($slug)
    {
        ...
        $key = 'user_products_user' . $data["user"]->id . 'cache_page_' . $pagination['current_page'];
        $data['products'] = get_cached_data($key);
        if (empty($data['products'])) {
            $data['products'] = $this->product_model->get_paginated_user_products($data["user"]->slug, $pagination['per_page'], $pagination['offset']);
            set_cache_data($key, $data['products']);
        }
    ...
    public function pending_products()
    {
        ...
        $key = 'user_pending_products_user' . $data["user"]->id . 'cache_page_' . $pagination['current_page'];
        $data['products'] = get_cached_data($key);
        if (empty($data['products'])) {
    ...
    public function reviews($slug)
    {
        ...
        $data['title'] = $data["user"]->username . " " . trans("reviews");
    ...
    public function preferences()
    {
        ...
        $data['description'] = trans("preferences") . " - " . $this->app_name;
        $data['keywords'] = trans("preferences") . "," . $this->app_name;
        $data["curr_user_role"] = get_current_user_role();
        $data["user"] = user();
        if (empty($data["user"])) {
            redirect(lang_base_url());
