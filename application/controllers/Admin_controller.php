...
class Admin_controller extends Admin_Core_Controller
{
    ...
    */
    ...
    {
        if ($this->settings_model->update_twitter_login()) {
            $this->session->set_flashdata('success', trans("msg_updated"));
            $this->session->set_flashdata("mes_social_twitter", 1);
            redirect($this->agent->referrer());
        } else {
            $this->session->set_flashdata('error', trans("msg_error"));
            $this->session->set_flashdata("mes_social_twitter", 0);
            redirect($this->agent->referrer());
        }
    }
    ...
    */
    ...
    public function stripe_settings_post()
    {
        ...
        if (!is_writable(APPPATH . 'config/stripe.php')) {
            $this->session->set_flashdata('error', "'application/config/stripe.php' file is not writable!");
        ...
        if ($this->settings_model->update_stripe_settings()) {
            ...
            //update stripe.php
            $stripe_vars = array();
            $stripe_vars["publishable_key"] = $this->input->post('stripe_publishable_key', true);
            $stripe_vars["secret_key"] = $this->input->post('stripe_secret_key', true);

            $start = '<?php defined("BASEPATH") OR exit("No direct script access allowed");' . PHP_EOL;
            $keys = '';
            $end = '?>';
            foreach ($stripe_vars as $key => $value) {
                $keys .= '$stripe_vars["' . $key . '"] = "' . $value . '";' . PHP_EOL;
            }
            $content = $start . $keys . $end;
            file_put_contents(APPPATH . 'config/stripe.php', $content);
