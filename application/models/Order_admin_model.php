...
class Order_admin_model extends CI_Model
{
    ...
    //update order payment as not received
    public function update_order_payment_not_received($order_id)
    {
        $order = $this->get_order($order_id);
        if (!empty($order)) {
            //update product payment status
    ...
    public function set_order_as_completed($order_id)
    {
        $order = $this->get_order($order_id);
        if (!empty($order)) {
            $data = array(
    ...
    //set order as not completed
    public function set_order_as_not_completed($order_id)
    {
        $order = $this->get_order($order_id);
        if (!empty($order)) {
            $data = array(
