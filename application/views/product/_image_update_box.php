        ...
        <?php for ($i = 1; $i < 6; $i++): ?>
            <div class="col-6 col-sm-4 col-md-3 col-image-input">
                <div class="image-input-box">
                    ...
                    <?php endif; ?>
                    <?php if ($images_array[$i]->id != ""): ?>
                        <div class="input-uploaded-image">
                            <img src="<?php echo $img_bg_product_small; ?>" alt="bg" class="img-fluid img-input-bg">
                            <a href="javascript:void(0)" class="btn-img-delete btn-delete-product-img" data-product-id="<?php echo $product->id; ?>" data-image-id="<?php echo $images_array[$i]->id; ?>"><i class="icon-times"></i></a>
                            <img src="<?php echo get_product_image_url($images_array[$i], 'image_small'); ?>" alt="" class="img-uploaded">
                        </div>
                    <?php else: ?>
                        <div class="top">
                            <?php echo form_open_multipart('file_controller/upload_image', ['id' => 'product_image_form_' . $i]); ?>
                            <img src="<?php echo $img_bg_product_small; ?>" alt="bg" class="img-fluid img-input-bg">
                            <input type="hidden" name="product_id" value="<?php echo $product->id; ?>">
                            <input type="hidden" name="image_order" value="<?php echo $i; ?>">
                            <a class='btn-input btn-product-image-upload'>
                                <i class="icon-plus"></i>
                                <input type="file" id="img_file_input_<?php echo $i; ?>" name="file" class="img-file-input" data-image-order="<?php echo $i; ?>" data-product-id="<?php echo $product->id; ?>" accept=".png, .jpg, .jpeg, .gif" required>
                            </a>
                            <?php echo form_close(); ?>
                        </div>
                    <?php endif; ?>
